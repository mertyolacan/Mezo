"use client";

import { useState } from "react";
import Link from "next/link";
import {
  MessageCircle,
  Phone,
  Mail,
  X,
  Headphones,
  MessageSquare,
} from "lucide-react";

interface Props {
  phone?: string | null;
  email?: string | null;
  whatsapp?: string | null;
}

const actions = (phone?: string | null, email?: string | null, whatsapp?: string | null) => [
  {
    label: "Telefon",
    icon: Phone,
    href: phone ? `tel:${phone}` : null,
    color: "bg-emerald-500 hover:bg-emerald-600",
    external: true,
  },
  {
    label: "E-posta",
    icon: Mail,
    href: email ? `mailto:${email}` : null,
    color: "bg-sky-500 hover:bg-sky-600",
    external: true,
  },
  {
    label: "WhatsApp",
    icon: () => (
      <svg viewBox="0 0 24 24" className="h-4 w-4 fill-current" xmlns="http://www.w3.org/2000/svg">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
      </svg>
    ),
    href: whatsapp ? `https://wa.me/${whatsapp.replace(/\D/g, "")}` : null,
    color: "bg-[#25D366] hover:bg-[#1ebe5d]",
    external: true,
  },
  {
    label: "Destek",
    icon: Headphones,
    href: "/profile/support",
    color: "bg-brand-secondary hover:bg-brand-secondary/90",
    external: false,
  },
  {
    label: "Mesaj",
    icon: MessageSquare,
    href: "/contact",
    color: "bg-brand-primary hover:bg-brand-primary/90",
    external: false,
  },
];

export default function ContactBubble({ phone, email, whatsapp }: Props) {
  const [open, setOpen] = useState(false);
  const items = actions(phone, email, whatsapp);

  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-center gap-2">
      {/* Action buttons */}
      <div
        className={`flex flex-col items-center gap-2 transition-all duration-300 ${
          open ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 translate-y-4 pointer-events-none"
        }`}
      >
        {[...items].reverse().map(({ label, icon: Icon, href, color, external }, i) => (
          href ? (
            external ? (
              <a
                key={label}
                href={href}
                target="_blank"
                rel="noopener noreferrer"
                title={label}
                style={{ transitionDelay: open ? `${i * 40}ms` : "0ms" }}
                className={`group relative flex items-center justify-center w-11 h-11 rounded-full text-white shadow-lg transition-all duration-200 ${color} ${
                  open ? "opacity-100 scale-100" : "opacity-0 scale-75"
                }`}
              >
                <Icon />
                <span className="absolute right-14 bg-zinc-900 dark:bg-zinc-700 text-white text-xs font-medium px-2.5 py-1 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  {label}
                </span>
              </a>
            ) : (
              <Link
                key={label}
                href={href}
                title={label}
                onClick={() => setOpen(false)}
                style={{ transitionDelay: open ? `${i * 40}ms` : "0ms" }}
                className={`group relative flex items-center justify-center w-11 h-11 rounded-full text-white shadow-lg transition-all duration-200 ${color} ${
                  open ? "opacity-100 scale-100" : "opacity-0 scale-75"
                }`}
              >
                <Icon />
                <span className="absolute right-14 bg-zinc-900 dark:bg-zinc-700 text-white text-xs font-medium px-2.5 py-1 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  {label}
                </span>
              </Link>
            )
          ) : null
        ))}
      </div>

      {/* Main toggle button */}
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label={open ? "Kapat" : "İletişim"}
        className={`flex items-center justify-center w-14 h-14 rounded-full shadow-xl transition-all duration-300 ${
          open
            ? "bg-zinc-700 hover:bg-zinc-800 rotate-0"
            : "bg-brand-primary hover:bg-brand-primary/90"
        } text-white`}
      >
        <span className={`absolute transition-all duration-200 ${open ? "opacity-100 scale-100" : "opacity-0 scale-75"}`}>
          <X className="h-5 w-5" />
        </span>
        <span className={`absolute transition-all duration-200 ${open ? "opacity-0 scale-75" : "opacity-100 scale-100"}`}>
          <MessageCircle className="h-6 w-6" />
        </span>
      </button>
    </div>
  );
}
